...
% parameters setting
 for ppt = 1:consts.npair
     winner_par = sprintf('W%s.csv','BR');
     loser_par = sprintf('L%s.csv','BR');
    ...
    for CurrentTrial=1:consts.ntrials
        ...
        if CurrentTrial == 1 %first trial
            ...
            [ChoiceProbP1R,ChoiceProbP1P,ChoiceProbP1S] = CalculateBRChoiceProb(P1DecisionC, P1WLT, wa1, wa2, wa3);
            [ChoiceProbP2R,ChoiceProbP2P,ChoiceProbP2S] = CalculateBRChoiceProb(P2DecisionC, P2WLT, la1, la2, la2);
    ...
    OutcomematP1(ppt,1:3) = mean(WTLMatP1,1)/100; %average number of win, tie, lose for P1
    OutcomematP2(ppt,1:3) = mean(WTLMatP2,1)/100; %average number of win, tie, lose for P2
    OutcomematP1(ppt,4:6) = std(WTLMatP1,1)/100;
    OutcomematP2(ppt,4:6) = std(WTLMatP2,1)/100;
