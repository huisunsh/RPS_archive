...
nlnLW_empirical = zeros(consts.npair,1);
nlnLL_empirical = zeros(consts.npair,1);

nlnLW_ne = zeros(consts.npair,1);
nlnLL_ne = zeros(consts.npair,1);
        
for ppt = 1:consts.npair
    W = winnerchoice(ppt,:);
    nlnLW_empirical(ppt,:) = baseline_empirical(W);

    nlnLW_ne(ppt,:) = baseline_ne(W);
end

for ppt = 1:consts.npair
    L = loserchoice(ppt,:); 
    nlnLL_empirical(ppt,:) = baseline_empirical(L);

    nlnLL_ne(ppt,:) = baseline_ne(L);
end
